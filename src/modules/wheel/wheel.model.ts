...
export const editWheelById = async (wheelIds: number[]) => {
	if (!wheelIds || wheelIds.length === 0) {
		return []
	}
...
export const createWheelInDb = async (wheelIds: number[]) => {
	if (!wheelIds || wheelIds.length === 0) {
		return []
	}
...
const updateMainWheelData = async (wheelId: number, wheelData: IWheelSave, client?: PoolClient) => {
	...
	const wheelResult = await db.query(updateWheelQuery, [wheelData.name, wheelData.interval_seconds, wheelId])

	return wheelResult.rows[0]
}

const getCurrentFieldsIdForWheel = async (wheelId: number) => {
	const existingFieldsResult = await pool.query('SELECT field_id FROM wheels_fields WHERE wheel_id = $1', [wheelId])
...
const checkWheelFieldLinkExists = async (wheelId: number, fieldId: number) => {
	const existingLink = await pool.query('SELECT 1 FROM wheels_fields WHERE wheel_id = $1 AND field_id = $2', [
		wheelId,
		fieldId
	])
...
const createWheelFieldLink = async (wheelId: number, fieldId: number, client?: PoolClient) => {
	const db = client || pool
	await db.query('INSERT INTO wheels_fields (wheel_id, field_id) VALUES ($1, $2)', [wheelId, fieldId])
}

const deleteWheelFieldLInk = async (wheelId: number, fieldId: number, client?: PoolClient) => {
	const db = client || pool
	await db.query('DELETE FROM wheels_fields WHERE wheel_id = $1 AND field_id = $2', [wheelId, fieldId])
}
...
export const updateWheel = async (wheelId: number, wheelData: IWheelSave): Promise<IWheel> => {
	...
	try {
		...
		for (const field of wheelData.fields) {
			const fieldId =
				(await getExistingFieldId(field.name, field.color_hex)) ||
				(await createNewField(field.name, field.color_hex, client))
...
export const createWheel = async (wheelData: IWheelSave, userId: number): Promise<IWheel> => {
	...
	try {
		...
		for (const field of wheelData.fields) {
			const fieldId =
				(await getExistingFieldId(field.name, field.color_hex)) || (await createNewField(field.name, field.color_hex))
			await createWheelFieldLink(wheelId, fieldId, client)
		}
		await client.query('COMMIT')
		const wheel: IWheel[] = await getWheelsByIdArray([wheelId])
