...
export const createFieldsFromArrary = async (fields: IField[], wheelId?: number): Promise<IField[]> => {
	for (const field of fields) {
		...
		if (wheelId) {
			await pool.query('INSERT INTO wheels_fields (wheel_id, field_id) VALUES ($1, $2)', [wheelId, field.field_id])
		}
	}
	return fields
}
...
export const updateFieldsWheelConnection = async (fields: IField[], wheelId: number): Promise<IField[]> => {
	const currentFieldsResult = await pool.query('SELECT field_id FROM wheels_fields WHERE wheel_id = $1', [wheelId])
	...
	for (const field of fields) {
		...
		if (!exists) {
			await pool.query('INSERT INTO wheels_fields (wheel_id, field_id) VALUES ($1, $2)', [wheelId, fieldId])
		}
	}
	const toDelete = currentFieldIds.filter((id) => !newFieldIds.includes(id))
	if (toDelete.length > 0) {
		await pool.query('DELETE FROM wheels_fields WHERE wheel_id = $1 AND field_id = ANY($2::int[])', [wheelId, toDelete])
	}
	return fields
}

const getExistFieldId = async (field: IField): Promise<boolean | number> => {
	const result = await pool.query('SELECT field_id FROM fields WHERE name = $1 and color_hex = $2 LIMIT 1', [
		field.name,
		field.color_hex
	])
