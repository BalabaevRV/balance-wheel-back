...
describe('Auth routes Integration Tests', () => {
	...
	describe('POST /api/signup', () => {
		test('should create new user successfully', async () => {
			const response = await request(app).post('/api/signup').send(userData).expect(201)
			...
			const dbResult = await pool.query('SELECT * FROM users WHERE login = $1', [userData.login])
